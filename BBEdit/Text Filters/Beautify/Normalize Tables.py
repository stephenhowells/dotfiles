...
def just(string, justification_type, n):
    "Justify a string to length n according to the justification type."
    if justification_type == '::':
        return string.center(n)
    elif justification_type == '-:':
...
def normtable(text):
    ...
    if formatline is None:
        raise ValueError("No separator line found in the table.")
    ...
    for cell in fstrings:
        ends = cell[0] + cell[-1] if len(cell) > 1 else ''
        if ends == '::':
            justify.append('::')
        elif ends == '-:':
            justify.append('-:')
    ...
    # Add whitespace to make all the columns the same width and justify them.
    formatted = []
    for row in content:
